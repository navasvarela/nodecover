...
var isForBlock = function(line) {
    return for_regex.test(line) && (line.indexOf('//') < 0 || (line.indexOf('//') > line.indexOf('for'))) && line.indexOf("'") < 0;
};
...
var getLastLineOfTryCatch = function(lineNum, docLines) {
	do { 
		lineNum++;
	} while(docLines[lineNum].indexOf('catch') < 0);
	do {
		lineNum++;
...
var wrapIfStatement = function(lineNum, docLines) {
	...
	if (line.indexOf('{') < 0) {
		...
		if (if_regex.test(line) && line.indexOf(';') > 0) {
		...
		} else if(isForBlock(docLines[lineNum+1])) {
			...
			if (docLines[lineNum+1].indexOf('try') >= 0) {
				lineNum = getLastLineOfTryCatch(lineNum+1,docLines);
			} else {
				do {
					lineNum++;
				} while(docLines[lineNum].indexOf(';') < 0);
