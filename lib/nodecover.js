...
var processLines = function(module) {
	...
	all_lines = cleaner.cleanForLoops(all_lines);
	all_lines.forEach(function(line) {
		...
		if (line.charAt(line.length - 1) === ';' && source_regex.test(line)) {
			...
			if (if_regex.test(line) && line.indexOf('{') < 0) {
				line = cleaner.wrapIfStatement(line);	
			}
...
var processDir = function(dir) {
	...
	for (i = 0; i < dirFiles.length; i++) {
		...
		if (stat && stat.isDirectory()) {
			processDir(file, listOfFiles);
		} else if (stat) {
			if (stat.isFile() && js_regex.test(file)) {
	...
	console.log('returning listOfFiles: '+JSON.stringify(listOfFiles));
	return listOfFiles;
};
exports.processDir = processDir;

var restoreFiles = function(files) {
	console.log('restoring files: '+JSON.stringify(files));
