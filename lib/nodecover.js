...
var lineCovered = function(module,line) {
	...
	winston.debug(__filename+':'+ JSON.stringify(Stats));
};
...
var blockCovered = function(module,line) {
	winston.debug(__filename+': blockCovered('+module+','+line+')');
...
var processBlock = function(module,line, lineNumber,totalBlocks) {
    winston.debug(__filename+': processBlock('+line+','+lineNumber+','+totalBlocks);	
	var result, fragments, index = 0;
	if (line.indexOf('{') >= 0) {
		result = '';
...
var processLines = function(module) {
	...
	fs.readFileSync(module+'.bak').toString().split('\n').forEach(function(line) {
		...
		winston.debug('functionLines: '+functionLines);
	...
	modified_lines.forEach(function(line) {
		winston.debug(__filename+"Writing to file: "+module+".mod, line: "+line);
...
var parseModule = function(module) {
	winston.debug('parsing module: '+module);
	...
	winston.debug('Instrumenting file: '+module);
