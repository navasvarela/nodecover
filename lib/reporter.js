...
var getModuleStats = function(mod,stats) {
	var report = '';
	report += ' '+ (stats.functions.total === 0 ? 100 : 100*stats.functions.hit.length / stats.functions.total) + '% ('+stats.functions.hit.length + '/'+ stats.functions.total+') |';           
	report += ' '+ (stats.blocks.total === 0 ? 100 : 100*stats.blocks.hit.length / stats.blocks.total) + '% ('+stats.blocks.hit.length + '/'+ stats.blocks.total+') |';   
	report += ' '+ (stats.lines.source === 0? 100 : 100*stats.lines.hit.length / stats.lines.source) + '% ('+stats.lines.hit.length+'/'+stats.lines.source+')';
	report += ' | '+mod + '\n';
	return report;
};
...
exports.printTxtReport = function(nodecover_stats) {
	...
	report += (totals.totalFunctions === 0 ? 100 : 100*totals.usedFunctions / totals.totalFunctions) + '% ('+totals.usedFunctions+'/'+totals.totalFunctions+') | ';
	report += (totals.totalBlocks === 0 ? 100 : 100*totals.usedBlocks / totals.totalBlocks) + '% ('+totals.usedBlocks+'/'+totals.totalBlocks+') | ';
	report += (totals.totalExecLines === 0 ? 100 : 100*totals.usedLines / totals.totalExecLines) + '% ('+totals.usedLines+'/'+totals.totalExecLines+')\n';
	report += '\n';
...
exports.printModuleReport = function(module) {
	...
	var module_file = path.normalize(process.cwd()+'/'+module);
	var module_stats = stats[module];
	var module_string = fs.readFileSync(module_file).toString();
	console.log('Printing module report for module: '+module_file);
	var lineNumber = 0;
